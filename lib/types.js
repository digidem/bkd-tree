...
var writers = {
  ...
  int8: function (buffer, value, offset, n) {
    if (typeof value === 'number') {
  ...
  int16be: function (buffer, value, offset, n) {
    if (typeof value === 'number') {
      return buffer.writeInt16BE(value, offset)
    }
    for (var i = 0; i < n; i++) {
      buffer.writeInt16BE(value[i], offset+i*2)
    }
  },
  int16le: function (buffer, value, offset, n) {
    if (typeof value === 'number') {
  ...
  int32be: function (buffer, value, offset, n) {
    if (typeof value === 'number') {
      return buffer.writeInt32BE(value, offset)
    }
    for (var i = 0; i < n; i++) {
      buffer.writeInt32BE(value[i], offset+i*4)
    }
  },
  int32le: function (buffer, value, offset, n) {
    if (typeof value === 'number') {
...
var parsers = {
  ...
  int8: function (buffer, offset, n) {
    if (n === 1) {
      return buffer.readInt8(offset)
    }
    if (isBigEndian) {
      return new Int8Array(buffer.buffer, offset, n)
    }
    var res = new Int8Array(n)
    for (var i = 0; i < n; i++) {
      res[i] = buffer.readInt8(offset+i*2)
    }
    return res
  },
  int16be: function (buffer, offset, n) {
    if (n === 1) {
      return buffer.readInt16BE(offset)
    }
    if (isBigEndian) {
      return new Int16Array(buffer.buffer, offset, n)
    }
    var res = new Int16Array(n)
    for (var i = 0; i < n; i++) {
      res[i] = buffer.readInt16BE(offset+i*2)
    }
    return res
  },
  int16le: function (buffer, offset, n) {
    if (n === 1) {
    ...
    var res = new Int16Array(n)
    for (var i = 0; i < n; i++) {
      res[i] = buffer.readInt16LE(offset+i*2)
    }
    return res
  },
  int32be: function (buffer, offset, n) {
    if (n === 1) {
      return buffer.readInt32BE(offset)
    }
    if (isBigEndian) {
      return new Int32Array(buffer.buffer, offset, n)
    }
    var res = new Int32Array(n)
    for (var i = 0; i < n; i++) {
      res[i] = buffer.readInt32BE(offset+i*2)
    }
    return res
  },
  int32le: function (buffer, offset, n) {
    if (n === 1) {
    ...
    var res = new Int32Array(n)
    for (var i = 0; i < n; i++) {
      res[i] = buffer.readInt32LE(offset+i*2)
    }
    return res
  }
}
